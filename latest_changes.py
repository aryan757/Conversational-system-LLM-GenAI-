...
# Dictionary to map user choices to anonymity
anonymity_options = {
    "Yes": True,
    "No": False
}
...
def display_follow_up_questions(intent_classification):
    if intent_classification in follow_up_questions:
        ...
        anonymity_choice = st.radio("Keep identity anonymous?", list(anonymity_options.keys()))
        return follow_up_responses, anonymity_options[anonymity_choice]
    else:
...
# Streamlit front-end
st.title("Police Service Portal System")
    ...
    if user_query:
        ...
        try:
            ...
            if intent_classification and intent_classification in follow_up_questions:
                ...
                for question in follow_up_questions[intent_classification]:
                    follow_up_responses[question] = st.text_input(question, key=question, value=follow_up_responses.get(question, ""))
                
                anonymity_choice = st.radio("Keep information anonymous?", list(anonymity_options.keys()))
                is_anonymous = anonymity_options[anonymity_choice]
                
                if st.button("Submit Follow-up Information"):
                    with open('user_data.csv', mode='a', newline='') as file:
                        writer = csv.writer(file)
                        if is_anonymous:
                            # Generate a hash of the user's identity and use it as an identifier
                            identity_hash = hashlib.sha256((user_name + user_phone).encode()).hexdigest()
                            writer.writerow([identity_hash, "Anonymous", "Anonymous", user_query, intent_classification, *follow_up_responses.values()])
                        else:
                            writer.writerow([
