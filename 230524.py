...
def display_map(location, key):
    ...
    if folium_map['last_clicked']:
        last_clicked = folium_map['last_clicked']
        folium.Marker(last_clicked).add_to(m)
        st.session_state[key] = last_clicked  # Save the clicked location in the session state
        st.write(f"You selected: {last_clicked}")  # Display the selected location
    return folium_map

# Function to display map and get location from user
def get_location_from_map(prompt, key, initial_coords=(0, 0), zoom_start=2):
    st.subheader(prompt)
    ...
    if folium_map['last_clicked']:
        last_clicked = folium_map['last_clicked']
        # Convert the last_clicked dictionary to a tuple (latitude, longitude)
        last_clicked_coords = (last_clicked['lat'], last_clicked['lng'])
        folium.Marker(last_clicked_coords).add_to(m)
        st.session_state[key] = last_clicked_coords  # Save the clicked location in the session state

        # Reverse geocoding to get the address from the coordinates
        geolocator = Nominatim(user_agent="streamlit_app")
        try:
            location = geolocator.reverse(last_clicked_coords, timeout=10)
            address = location.address
            st.write(f"You selected: {last_clicked_coords}")
            st.write(f"Address: {address}")  # Display the address
    ...
    return st.session_state.get(key, initial_coords)  # Return the actual location captured from the marker
...
current_location = get_location_from_map(
    "Pinpoint your current location:",
    current_location_key,
    geocoder.ip('me').latlng if geocoder.ip('me').latlng else [20, 78],
    4
)
...
incident_location_key = "incident_location"
incident_location = get_location_from_map(
